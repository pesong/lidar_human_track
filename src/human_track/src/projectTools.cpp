...
void getBox(geometry_msgs::Point32 &it, Point2d &pt2D, Rect &rect, double &M_TO_PIXELS, double &BODY_RATIO) {
    ...
    // Compute the rectangle
     rect.x = (int)pt2D.x - (int)(boxWidth / 2);
     rect.y = (int)(pt2D.y - BODY_RATIO*(1-0.05*sqrt((dist-3)*(dist-3))) * boxWidth);
     rect.width = (int)(1.0 * boxWidth);
     rect.height = (int)(2.0 * boxWidth);
    //csy
 /*   if ((int) (pt2D.x - (boxWidth / 2) >= 0 && (pt2D.x + (boxWidth / 2)) < 752)) {
        rect.x = (int) (pt2D.x - (boxWidth / 2));
        rect.width = (int) (1.0 * boxWidth);
    } else

        if ((int)(pt2D.x - (boxWidth / 2)) < 0) {
            rect.x = 0;
            rect.width = (int) (1.0 * boxWidth);
        } else {
        rect.x = (int) (pt2D.x - (boxWidth / 2));
        rect.width = (int) (1.0 * boxWidth) - ((pt2D.x + (boxWidth / 2)) - 752);
    }
    if ((int)(pt2D.y - BODY_RATIO * boxWidth) >= 0 && (int)(pt2D.y) < 480) {

        rect.y = (int) (pt2D.y - BODY_RATIO * boxWidth);
        rect.height = (int) (BODY_RATIO * boxWidth);
    } else if ((int)(pt2D.y - BODY_RATIO * boxWidth) < 0) {
        rect.y = 0;
        rect.height = (int) (BODY_RATIO * boxWidth)>480? 0 : (int) (BODY_RATIO * boxWidth);
    }
    else{
        rect.y = 10;
        rect.height = 460;
    }*/
   /* if ((pt2D.x - (boxWidth / 2) > 0 && (pt2D.y - BODY_RATIO * boxWidth) > 0 && (pt2D.x + (boxWidth / 2)) > 752)) {

        rect.x = (int) 0; //(pt2D.x - (boxWidth / 2));
        rect.y = (int) 0; //(pt2D.y - BODY_RATIO * boxWidth);
        rect.width = (int) 752; //(1.0 * boxWidth);
        rect.height = (int) 480; //(2.0 * boxWidth);
        printf("\n--------------\n");*/
   ...
   // }
    ...
    /*    else{
                  //csy begin
                  rect.x = (int)(pt2D.x - (4.0));
                  rect.y = (int)(pt2D.y - 8.0);
                  rect.width = (int)(4.0);
                  rect.height = (int)(8.0);
        }*/
    //csy end
