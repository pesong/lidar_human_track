...
//        demoClasses_ = classes;
        ...
        objectPublisher_ = nodeHandle_.advertise<std_msgs::Int8>(objectDetectorTopicName,
                                                                 ...
                                                                 objectDetectorLatch);
        boundingBoxesPublisher_ = nodeHandle_.advertise<human_track::BoundingBoxes>(
...
//                camImageCopy_ = cv::cvarrToMat(frame, true);
    ...
    bool SSD_Detector::publishDetectionImage(const cv::Mat &detectionImage) {
        ...
        //std::cout << "detectionImage: " << detectionImage << std::endl;
...
//            probs_[j] = (float *) calloc(l.classes + 1, sizeof(float));
    ...
    cv::Mat SSD_Detector::getCVImage() {
       // std::cout << "getCVImage" << std::endl;
        ...
        //std::cout << "camImageCopy_0: " << camImageCopy_ << std::endl;
       // std::cout << "ROS_img width: " << ROS_img.cols << std::endl;
        return ROS_img;
    }
    ...
    //选出laser里是人的
    std::vector<int> SSD_Detector::select_humans(std::vector<cameraRect> Rects_camera, ros::Time time_u)
    {
        mutexLaserSet.lock();
        //这是一个buff，对比每一个框，找到id, 找到id后返回humans，找humans在主线程里做
        std::deque<laserSet> laser_sets = Laser_sets();
        mutexLaserSet.unlock();
        double time_th = 0.15;
        double time_diff = time_th;
        laserSet select_set;
        std::vector<int> select_id;
        //std::cout << "Laser_sets.size" << Laser_sets().size() << std::endl;
        //先找时间相近的一帧
        for(auto it = laser_sets.begin(); it != laser_sets.end(); it++) {
//            std::cout << "laser time: " << (it->timestamp)<< std::endl;
//            std::cout << "camera time: " << time_u << std::endl;
//            std::cout << "time now: " << ros::Time::now() << std::endl;
//            if(((it->timestamp).toSec()-time_u.toSec()) < 0) {
//               ROS_ERROR("The time match of camera and laser is wrong");
//               break;
//            }

            if(fabs((it->timestamp).toSec()-time_u.toSec()) < time_diff) {
                select_set = *it;
                time_diff = (it->timestamp).toSec()-time_u.toSec();
            }
        }
        //ROS_WARN("time_diff: %f", time_diff);

        std::vector<cv::Rect> laser_rects0;
        //std::cout << "aaaa:" << select_set.laser_rects.size() << std::endl;
        for(int i=0; i < select_set.laser_rects.size(); i++) {
            for(int j=0; j < Rects_camera.size(); j++)
            {
               if(Rects_camera.size()!=0 && select_set.laser_rects.size()!=0) {
                   //getOverlap(select_set) Rects_camera[j], select_set.laser_rects[i]

                   laser_rects0.push_back(select_set.laser_rects[i].rect);

                   if (getOverlap(select_set.laser_rects[i].rect, Rects_camera[j].rect) > 0.5)
                       select_id.push_back(select_set.laser_rects[i].id); //选中的人物id push进去
               }
            }
        }
        laser_rects = laser_rects0;
        //std::cout << "aaaab:" << laser_rects.size() << std::endl;
        //std::cout << "select_id.size: " << select_id.size() << std::endl;
        return select_id; //有可能size==0,就是这帧里没有人
...
//        //human_track::BoundingBox boundingBox;
//        if (num > 0 && num <= 100) {
//            for (int i = 0; i < num; i++) {
...
//
//            for (int i = 0; i < numClasses_; i++) { //对每一类
