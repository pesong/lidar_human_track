...
private:
   ...
   */
  void features2ROS(std::vector<human_track::ClusteredScan> &clusterData);
...
public:
   ...
   * @param features[out] Exported cluster features
   */
  void getFeatures(std::vector<human_track::ClusteredScan> &clusterData);

  /**
   *
   * @param features[out] Where the clusters are going to be exported.
   */
  void getClusters(std::vector<human_track::ClusteredScan> &clusterData);
