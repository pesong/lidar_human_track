...
public:
    void initParams();
    void init(std::vector<cv::Size> & scales);
    
    int getNumTrees() const { return nstructs; }
    void calcFeatures(cv::Mat & patch, int const& scale_idx, std::vector<int>& fern);
    ...
    int getNstructs() const {return nstructs; };
    int getStructSize() const {return structSize; };
...
private:
    ...
    struct Feature {
        ...
        float x1, y1, x2, y2;
        bool operator()(cv::Mat & patch) const { return patch.at<uchar>(y1,x1) > patch.at<uchar>(y2,x2); }
    };
