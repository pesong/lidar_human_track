...
void Ferns::init(std::vector<cv::Size> & scales)
{
    this->clear();
...
void Ferns::calcFeatures(cv::Mat & patch, int const& scale_idx, std::vector<int>& fern)
{
    int leaf;
    auto fff = features.at(scale_idx).begin();
    for (int t=0; t<nstructs; t++){
        leaf = 0;
        for (int f=0; f<structSize; f++){
            //usleep(10); 
            //leaf = (leaf << 1) + features.at(scale_idx).at(t*nstructs+f)(patch);
            leaf = (leaf << 1) + (*fff++)(patch);
        }
        fern[t] = leaf;
    }
}
...
/*    cv::RNG& rng = cv::theRNG();*/
    ...
    double rng;
    for (int i=0; i<totalFeatures; i++) {
        rng = d(e);while(rng<0 || rng >1) rng=d(e);
        float x1 = static_cast<float>(rng);
        rng = d(e);while(rng<0 || rng >1) rng=d(e);
        float y1 = static_cast<float>(rng);
        rng = d(e);while(rng<0 || rng >1) rng=d(e);
